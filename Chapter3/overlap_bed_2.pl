...
#perl /scale_wlg_nobackup/filesets/nobackup/ga02470/prevent_from_delete/script/ovelap_.bed.pl dir
...
foreach $a (@folders) {
  ...
  `bedtools intersect -a $dir$a/phased_axx.bed -b $dir$a/phased_axp.bed | bedtools intersect -a - -b $dir$a/phased_axg.bed | bedtools intersect -a - -b $dir$a/filter_axx500.bed | sort |  uniq -c | awk '{print \$2,\$3,\$4}' | sed 's/ /\t/g' > $dir$a/overlap.bed`;
  ...
  if (-z "$dir$a/overlap.bed") {
    #print "This is an empty file";
  }else{
    print "worked for $dir$a\n";
  }
